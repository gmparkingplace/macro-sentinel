...
{{
  ...
  "scenario_bull":"강세 시나리오를 2줄 이상으로. 반드시 현재 데이터와 다른 방향의 전개를 가정할 것. 발생 조건(예: VIX XX 이하, WTI $XX 이하)과 예상 S&P 500 목표 수준 포함.",
  "scenario_base":"기본 시나리오를 2줄 이상으로. 현재 추세가 지속될 때의 전개. 향후 4주 예상 경로 포함.",
  "scenario_bear":"약세 시나리오를 2줄 이상으로. 반드시 강세/기본과 다른 구체적 리스크 트리거 명시. 하방 목표 수준 포함.",
  "entry_triggers": [
    "AVOID/WAIT 판정 시: 진입을 고려할 수 있는 조건 완화 트리거 3가지 (예: VIX XX 이하 안정, WTI $XX 이하, F&G XX 이상 회복 등 구체적 수치 포함)",
    "각 트리거는 현재 데이터 기준으로 얼마나 달성됐는지 언급",
    "NOW 판정 시: 진입 후 청산을 고려해야 할 무효화 조건 3가지"
  ]
