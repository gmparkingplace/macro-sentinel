...
def calc_scores(d):
    ...
    # ── Fear & Greed ──────────────────────
    # 이전: <40 green, <60 yellow  → 개선: <35 green, <55 yellow
    if fg is not None:
    ...
    # ── 유가 (신규) ────────────────────────
    # WTI $75 이하 green, $85 이하 yellow, 초과 red
    if wti is not None:
...
def groq_analysis(d, scores):
    ...
    if scores.get("stagflation_risk"):
        stag_warning = "\n⚠️ STAGFLATION ALERT: WTI > $85 + 실업률 > 4.3% 동시 발생. 스태그플레이션 리스크 높음.\n"

    prompt = f"""당신은 월스트리트 수석 매크로 전략가입니다. 아래 실시간 뉴스 헤드라인과 시장 데이터를 함께 분석하여 심층 판단을 제공하세요.
...
- VIX: {d['indices']['vix']['close']} → {scores['vix'].upper()} (22 이하=green, 22~28=yellow, 28+=red 기준)
...
- WTI Oil: ${d['commodities']['wti']['close']} → {scores['oil'].upper()}
...
[스코어카드]
VIX:{scores['vix']} | 금리곡선:{scores['curve']} | HY:{scores['hy_spread']} | 금리:{scores['rates']} | DXY:{scores['dxy']} | TIPS:{scores['tips']} | 섹터:{scores['sector']} | 심리:{scores['sentiment']} | 고용:{scores['unemployment']} | 유가:{scores['oil']} | GDP:{scores['gdp']}
종합판정: {scores['verdict']} (ratio: {scores['ratio']})

━━━ 분석 지시 ━━━
- 뉴스 헤드라인에서 파악한 지정학·정책 리스크를 반드시 분석에 반영하세요
- 숫자만 보지 말고 뉴스 맥락을 통해 왜 이 수치가 나왔는지 설명하세요
- 스태그플레이션 리스크가 감지된 경우 이를 명확히 경고하세요
- 순수 JSON만 출력하세요 (마크다운 없이):

{{"section0_summary":"현재 시장 좌표: 뉴스 맥락 포함 3줄. 수치와 원인 모두 서술.","section1_fed":"Fed·금리: 현재 통화정책 스탠스, 금리곡선, 실질금리가 밸류에이션에 미치는 영향. 뉴스 반영.","section2_flow":"달러·자금흐름: DXY 방향과 원인, 글로벌 자금 이동 방향. 3줄.","section3_sector":"섹터 로테이션: 4주 누적 기준 강한/약한 섹터, Risk-On/Off 신호. 뉴스 맥락 포함.","section4_risk":"리스크: 뉴스에서 파악한 지정학·정책 리스크, 꼬리 리스크 포함 3줄.","bull_case":"강세 논거 2가지 (구체적 수치와 함께)","bear_case":"약세 논거 2가지 (구체적 수치와 함께, 뉴스 맥락 포함)","verdict_reason":"판정 {scores['verdict']}의 핵심 이유 2줄. 뉴스 기반 맥락 필수.","scenario_bull":"강세 시나리오: 어떤 조건이 갖춰지면 상승할지. 구체적 트리거.","scenario_base":"기본 시나리오: 현재 흐름 지속 시 예상 방향.","scenario_bear":"약세 시나리오: 어떤 리스크 현실화 시 하락할지. 구체적 트리거.","entry_triggers":["구체적 수치 포함된 진입 트리거 1","진입 트리거 2","진입 트리거 3"],"key_events":[{{"date":"YYYY-MM-DD","event":"이벤트명","impact":"예상 시장 영향"}},{{"date":"YYYY-MM-DD","event":"이벤트명","impact":"예상 시장 영향"}},{{"date":"YYYY-MM-DD","event":"이벤트명","impact":"예상 시장 영향"}}]}}"""
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "당신은 매크로 투자 전략가입니다. 뉴스 맥락과 시장 데이터를 종합하여 분석합니다. 요청한 JSON 형식만 출력하고 다른 텍스트는 절대 포함하지 마세요. 모든 분석은 한국어로 작성하세요."
                },
